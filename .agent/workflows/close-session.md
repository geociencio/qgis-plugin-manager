...
*   **`docs/CHANGELOG.md`**: Record user-visible changes in `[Unreleased]` using the **changelog-generator** skill based on session commits. **(NOTE: The `changelog-generator` skill is interpretive [prompt-based]. Do NOT attempt to run any Python script. Instead, use `git log` to read the session's recent commits, analyze them mentally, and use the file editing tool to write directly into CHANGELOG.md)**.
